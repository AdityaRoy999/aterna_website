...
// FluidGlass removed — stubbed to avoid heavy 3D dependencies and runtime errors.
  ...
  return (
    <>
      ...
      <mesh scale={[vp.width, vp.height, 1]}>
        <planeGeometry />
        <meshBasicMaterial map={(buffer as any).texture} transparent />
      </mesh>
      <mesh
        ref={ref}
        scale={scale ?? 0.15}
        ...
        {...props}
      >
        {/* MeshTransmissionMaterial exists in recent drei releases; install @react-three/drei >= v9 */}
        <MeshTransmissionMaterial
          buffer={(buffer as any).texture}
          ior={ior ?? 1.15}
...
function Images() {
  ...
  useFrame(() => {
    if (!group.current) return;
    (group.current.children[0].material as any).zoom = 1 + data.range(0, 1 / 3) / 3;
    (group.current.children[1].material as any).zoom = 1 + data.range(0, 1 / 3) / 3;
    (group.current.children[2].material as any).zoom = 1 + data.range(1.15 / 3, 1 / 3) / 2;
    (group.current.children[3].material as any).zoom = 1 + data.range(1.15 / 3, 1 / 3) / 2;
    (group.current.children[4].material as any).zoom = 1 + data.range(1.15 / 3, 1 / 3) / 2;
  });
